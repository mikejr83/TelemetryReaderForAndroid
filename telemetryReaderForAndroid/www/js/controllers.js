angular.module('telemetryReaderForAndroid.controllers', ['telemetryReaderForAndroid.services'])
  ...
  .controller('TelemetryViewerController', ['$scope', '$window', '$ionicLoading', '$ionicScrollDelegate', 'dataService',
      ...
      $scope.selectedFlightChanged = function () {
        ...
        console.log('dataSet', $scope.service.selectedFlight.flightData[$scope.service.selectedKey]);
          if($scope.chart) {
              
          } else {
              $scope.service.selectedFlight.flightData[$scope.service.selectedKey].element = 'myChart';
              $scope.chart = Morris.Line($scope.service.selectedFlight.flightData[$scope.service.selectedKey]);
          }
